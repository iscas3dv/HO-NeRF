...
from halo_util.utils import xyz_to_xyz1, pad34_to_44
...
def forward_get_3djoint_use_bone_and_bone_length(local_coords, bone_lengths, root):
    ...
    for k in range(n_fingers):
        ...
        for i in range(n_b_per_f):
            ...
            local_bone = (local_coords[:,idx] * bone_lengths[:,idx])
            target_point = start_point + local_bone
            joint_3d_pred.append(target_point)
            start_point += (local_bone)

    joint_3d_pred = torch.stack((joint_3d_pred),1)
    return joint_3d_pred
...
class PoseConverter(nn.Module):
    ...
    def __init__(self, store=False, dev=None, straight_hand=True, canonical_pose=None):
        ...
        self.initial_bone_vec = torch.tensor([[[ 4.4889e-01, -8.4880e-01, -2.7935e-01],
         [ 1.9867e-01, -9.8007e-01,  0.0000e+00],
         [ 2.0004e-07, -1.0000e+00,  0.0000e+00],
         [-1.9471e-01, -9.8007e-01, -3.9469e-02],
         [-3.7001e-01, -9.2185e-01, -1.1528e-01],
         [ 4.4889e-01, -8.4880e-01, -2.7935e-01],
         [ 1.9867e-01, -9.8007e-01,  1.1921e-07],
         [ 2.8685e-07, -1.0000e+00,  0.0000e+00],
         [-1.9471e-01, -9.8007e-01, -3.9470e-02],
         [-3.7001e-01, -9.2185e-01, -1.1528e-01],
         [ 4.4889e-01, -8.4880e-01, -2.7935e-01],
         [ 1.9867e-01, -9.8007e-01,  1.4901e-07],
         [ 1.9870e-06, -1.0000e+00,  2.3842e-07],
         [-1.9471e-01, -9.8007e-01, -3.9470e-02],
         [-3.7001e-01, -9.2185e-01, -1.1528e-01],
         [ 4.4889e-01, -8.4880e-01, -2.7935e-01],
         [ 1.9867e-01, -9.8007e-01,  8.9407e-08],
         [-3.4117e-06, -1.0000e+00, -2.1979e-07],
         [-1.9471e-01, -9.8007e-01, -3.9469e-02],
         [-3.7001e-01, -9.2185e-01, -1.1528e-01]]]).to(dev)

    # def change_canonicalpose(self, canonical_pose=None):
    ...
    def compute_local_coordinate_system(self, bones):
        ...
        rb_idx = self.rb_idx  # 0~4
        # rb_idx_bin = self.rb_idx_bin
        nrb_idx_list = self.nrb_idx_list  # 10~14, 15~19
        one = self.one
        eps = self.eps
        eps_mat = self.eps_mat
        xz_mat = self.xz_mat
        ...
        plane_normals = torch.cross(root_bones[:,:-1], root_bones[:,1:], dim=2)  # [1,4,3]
    ...
    def compute_rotation_matrix(self, rot_angles, bone_local, joint_refine_angle):
        ...
        abduction_angle = abduction_angle.reshape(batch_size, bone)
        abduction_angle[:,5:10] += joint_refine_angle[:,:5]
        abduction_angle = abduction_angle.reshape(batch_size * bone, 1)
        r_1 = rotation_matrix(abduction_angle, rotated_x)
        
        flexion_angle = (-rot_angles_flat[:, 0] + canonical_rot_flat[:, 0]).unsqueeze(1)
        flexion_angle = flexion_angle.reshape(batch_size, bone)
        flexion_angle[:,5:] += joint_refine_angle[:,5:]
        flexion_angle = flexion_angle.reshape(batch_size * bone, 1)
    ...
    def normalize_root_planes(self, bones, bone_lengths, palm_refine_angle):
        # root = torch.zeros([3])
        # fig = plt.figure()
        ...
        x = n0_n1_angle - canonical_angle[0] + palm_refine_angle[:,0]
        thumb_trans = rotation_matrix(n0_n1_angle - canonical_angle[0] + palm_refine_angle[:,0], bones_1)
        ...
        ring_trans = rotation_matrix(n2_n1_angle + canonical_angle[1] + palm_refine_angle[:,1], bones_2)
        ...
        pinky_trans = rotation_matrix(n3_n2_angle + canonical_angle[2] + palm_refine_angle[:,2], bones_3)
        ...
        return new_bones, root_plane_norm_mat, torch.cat([n0_n1_angle,n2_n1_angle,n3_n2_angle], 0)

    def normalize_root_bone_angles(self, bones, bone_lengths, palm_refine_angle):        
        # root = torch.zeros([3])
        # fig = plt.figure()
        ...
        index_trans = rotation_matrix(canonical_angle[1] - f2_f1_angle + palm_refine_angle[:,3], n1)
        ...
        thumb_trans = rotation_matrix(canonical_angle[0] - f1_f0_angle+ palm_refine_angle[:,4], n0)
        ...
        ring_trans = rotation_matrix(f3_f2_angle - canonical_angle[2]+ palm_refine_angle[:,5], n2)
        ...
        pinky_trans = rotation_matrix(f4_f3_angle - canonical_angle[3]+ palm_refine_angle[:,6], n3)
        ...
        return new_bones, root_angle_norm_mat, torch.cat([-f2_f1_angle,-f1_f0_angle,f3_f2_angle,f4_f3_angle], 0)

    def forward(self, joints, kp3d_is_right, 
                joint_refine_angle=None, palm_refine_angle=None,return_rot_only=False):

        assert joints.size(1) == 21, "Number of joints needs to be 21"

        if joint_refine_angle == None:
            joint_refine_angle = torch.zeros(joints.shape[0],20).to(joints.device)
        if palm_refine_angle == None:
            palm_refine_angle = torch.zeros(joints.shape[0],7).to(joints.device)
        ...
        plane_normalized_bones, root_plane_norm_mat, palm_a1 = self.normalize_root_planes(bones, bone_lengths, palm_refine_angle)

        angle_normalized_bones, root_angle_norm_mat, palm_a2 = self.normalize_root_bone_angles(plane_normalized_bones, bone_lengths, palm_refine_angle)
        ...
        rot_mat = self.compute_rotation_matrix(rot_angles, local_coords, joint_refine_angle)
        ...
        return trans_mat , 0, 0 # rot_mat # rot_angles # angle_loss



    def get_refine_3d_joint(self, joints, kp3d_is_right, mean_bone_length,
                        joint_refine_angle=None, palm_refine_angle=None,return_rot_only=False):

        assert joints.size(1) == 21, "Number of joints needs to be 21"
        batch_size = joints.shape[0]
        if joint_refine_angle == None:
            joint_refine_angle = torch.zeros((batch_size,20)).to(joints.device)
        if palm_refine_angle == None:
            palm_refine_angle = torch.zeros((batch_size,7)).to(joints.device)
        ...
        plane_normalized_bones, root_plane_norm_mat,_ = self.normalize_root_planes(bones, bone_lengths, palm_refine_angle)
        angle_normalized_bones, root_angle_norm_mat,_ = self.normalize_root_bone_angles(plane_normalized_bones, bone_lengths,palm_refine_angle)
        bones = angle_normalized_bones
        ...
        # rot_mat_new = self.compute_rotation_matrix_with_refine_angle(rot_angles, local_coords, joint_refine_angle)
        rot_mat_new = self.compute_rotation_matrix(rot_angles, local_coords, joint_refine_angle)
        loacl_cs_transpose_new = self.compute_adjusted_transpose(local_cs, rot_mat_new)
        trans_mat_without_scale_translation_new = torch.matmul(loacl_cs_transpose_new, torch.matmul(rot_mat_new, local_cs))

        rot_tpose_2_new = torch.matmul(trans_mat_without_scale_translation_new[...,:3,:3],root_bones_norm_mat)
        rot_tpose_2_new = torch.inverse(rot_tpose_2_new)
        p_bone = torch.matmul(rot_tpose_2_new, self.initial_bone_vec.unsqueeze(-1))[...,0]  # [B,20,3]

        root = torch.zeros([p_bone.shape[0],3]).to(joints.device)
        mean_bone_length = mean_bone_length.unsqueeze(-1)

        joint_3d = forward_get_3djoint_use_bone_and_bone_length(p_bone, mean_bone_length, root)

        return joint_3d # rot_mat # rot_angles # angle_loss
